...
public class Main {
    ...
    private static void runChecks(CheckerConfig config, File workingDir, CommandLineArgs cli) {
        ...
        File reportFile = new File(workingDir, "report.html");
        try {
            String htmlWithBom = "\ufeff" + html;
            java.nio.file.Files.write(reportFile.toPath(), htmlWithBom.getBytes(java.nio.charset.StandardCharsets.UTF_8));
            ...
            log.warning("Could not open report in browser: " + e.getMessage());
        }
